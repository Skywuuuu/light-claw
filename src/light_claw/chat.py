...
class ChatService:
    ...
    async def handle_message(self, message: FeishuInboundMessage) -> None:
        if not self._is_allowed_user(message.owner_id):
            return
        if not self.store.remember_inbound_message("feishu:" + message.message_id):
            return
    ...
    async def _handle_command(
        ...
        if command.kind == "reset":
            self.store.clear_session(message.conversation_id)
        ...
        if command.kind == "cli_use":
            ...
            self.workspace_manager.ensure_workspace_layout(updated)
            return "\n".join(
                [
        ...
        if command.kind == "workspace_list":
            ...
            workspaces = self.store.list_workspaces(message.owner_id)
        ...
        if command.kind == "workspace_current":
            ...
            state = self.store.get_conversation_state(message.conversation_id)
            return "\n".join(
                [
                    "Current workspace:",
        ...
        if command.kind == "workspace_create":
            ...
            record = self.workspace_manager.create_workspace(
                ...
                cli_provider=self.cli_registry.default_provider_id(
                    self.settings.default_cli_provider
                ),
            )
            record = self.store.create_workspace(record)
            self.workspace_manager.ensure_workspace_layout(record)
            self.store.set_current_workspace(
                message.conversation_id, message.owner_id, record.workspace_id
            )
            return "\n".join(
                [
        ...
        if command.kind == "workspace_use":
            ...
            self.store.set_current_workspace(
                message.conversation_id, message.owner_id, target.workspace_id
            )
            return "\n".join(
                [
                    "Workspace selected.",
    ...
    async def _handle_prompt(self, message: FeishuInboundMessage) -> None:
        workspace = self._ensure_current_workspace(
            owner_id=message.owner_id,
            conversation_id=message.conversation_id,
        )
    ...
    def _ensure_current_workspace(
        ...
        if state and state.workspace_id:
            workspace = self.store.get_workspace(owner_id, state.workspace_id)
            if workspace:
                self.workspace_manager.ensure_workspace_layout(workspace)
                return workspace

        workspaces = self.store.list_workspaces(owner_id)
        if workspaces:
            self.store.set_current_workspace(
                conversation_id, owner_id, workspaces[0].workspace_id
            )
            self.workspace_manager.ensure_workspace_layout(workspaces[0])
            return workspaces[0]
        ...
        self.workspace_manager.ensure_workspace_layout(created)
        self.store.set_current_workspace(conversation_id, owner_id, created.workspace_id)
        return created
    ...
    def _is_allowed_user(self, owner_id: str) -> bool:
        allow_from = self.settings.allow_from.strip()
